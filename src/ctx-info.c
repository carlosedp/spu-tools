...
{
	float ret = ((tot_t) > 0) ? ((new_t) - (old_t)) * 100.0 / (tot_t) : 0.0;
...
static int ctxs_compare(const void *v1, const void *v2)
{
	...
	{
	case CTX_PPU_PID:
		ret = p1->ppu_pid - p2->ppu_pid;
		break;
	case CTX_THREAD_ID:
		ret = p1->thread_id - p2->thread_id;
		break;
	case CTX_USER:
		ret = strcmp(p1->user, p2->user);
		break;
	case CTX_STATUS:
		ret = p1->status - p2->status;
		break;
	case CTX_FLAGS:
		ret = p1->flags - p2->flags;
		break;
	case CTX_PERCENT_SPU:
		ret = p1->percent_spu - p2->percent_spu;
		break;
	case CTX_SPE:
		ret = p1->spe - p2->spe;
		break;
	case CTX_TOTAL_TIME:
		ret = p1->time[TIME_TOTAL] - p2->time[TIME_TOTAL];
		break;

	case CTX_USER_TIME:
		ret = p1->time[TIME_USER] - p2->time[TIME_USER];
		break;
	case CTX_SYSTEM_TIME:
		ret = p1->time[TIME_SYSTEM] - p2->time[TIME_SYSTEM];
		break;
	case CTX_LOADED_TIME:
		ret = p1->time[TIME_LOADED] - p2->time[TIME_LOADED];
		break;

	case CTX_BINARY_NAME:
		ret = strcmp(p1->binary_name, p2->binary_name);
		break;
...
process_ctx_entry(struct dirent *entry, const char *gang_name, u64 last_period)
{
	...
	if (fp)
	{
		...
		ctx->time[TIME_TOTAL] = ctx->time[TIME_USER] + ctx->time[TIME_SYSTEM] + ctx->time[TIME_IOWAIT];
